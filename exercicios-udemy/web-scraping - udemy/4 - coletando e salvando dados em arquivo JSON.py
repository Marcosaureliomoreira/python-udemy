...
# armazenando na variável post, a class 'post' com o método find():

'''
post = soup.find(class_='post')

# acessando a class "post" de h3:
titulo = post.find('h3')
#print(titulo)

# acessando a class "post" do paragrafo:
texto = post.find('p').text
#print(texto)
...
# # armazenando na variável posts, a class 'post' com o método find_all():

posts = soup.find_all(class_='post')
postagens = []
for post in posts:
    titulo = post.find('h3').text
    texto = post.find('p').text
    link = post.find('a')['href']
    #fazendo o append de um dicionario:
    postagens.append({'titulo': titulo, 'texto': texto, 'link': link})

# criando um novo dicionário para passar o dicionário postagens:
dic_posts = {'posts': postagens}
#print(dic_posts)

# acessando titulo:
#print(dic_posts['posts'][0]['titulo'])

# acessando texto:
#print(dic_posts['posts'][0]['texto'])

# acessando link:
#print(dic_posts['posts'][0]['link'])


# TRANFORMANDO AS INFORMAÇÕES EM UM ARQUIVO JSON:

import json

with open('json/posts.json', 'w') as arquivo_json:
    json.dump(dic_posts, arquivo_json, indent=2)
